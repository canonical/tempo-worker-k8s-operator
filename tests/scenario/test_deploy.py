...
from scenario import Container, State, Relation
...
def test_pebble_ready_plan(ctx, relations):
    ...
    state_out = ctx.run(mimir_container.pebble_ready_event,
                        state=State(
                            containers=[mimir_container],
                            relations=list(Relation(endpoint,
                                                    interface='mimir_worker',
                                                    remote_app_name=f"{endpoint} app") for endpoint in mimir_relations)
                        ))
